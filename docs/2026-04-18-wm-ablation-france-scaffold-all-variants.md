...
**Not in this run:** `gru_multi` (multi-head GRU with auxiliary 14d/28d heads — see `forecast_charter.md` § WM skeleton). The CLI keyword `all` expands to **only** the four variants above; `gru_multi` is a separate variant and was never executed in this session. To include it, pass it explicitly, e.g. `--variant gru_multi` or `--variant linear,gru,gnn,gru_gnn,gru_multi`.
...
| `gru_multi` | *(not run in this session)* | Multi-head occurrence GRU (7d + aux horizons); `variant="gru_multi"`. Excluded from `--variant all` — pass `--variant gru_multi` or list it explicitly to run. |
