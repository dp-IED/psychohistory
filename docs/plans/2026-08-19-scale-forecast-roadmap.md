...
**Home scrape (unparked 2026-08-25):** user is home. LFI still had **no year playlist id** that morning; several 21 Aug watch URLs were empty livestream shells. **Do it now** from a residential IP: known ids + new channel uploads, playlist if it exists. Fold new public questions into `references/party-sources.md`; open further live problems only if anti-cluster still holds. Hallway notes from being on site stay in chat unless they are already on a public page. Continuity: `docs/plans/ce-handoff-lfi-dsa-transcripts.md`.
