...
This does not add a quality gate (ADR 0005 still holds: no transfer speech required to open a problem) and does not change who writes claims (ADR 0009: discover opens, predict forecasts). It only shapes which domains discover is allowed to cluster in in a single tick, so reflection has cross-domain series to test whether a method generalizes instead of one narrow vein.
