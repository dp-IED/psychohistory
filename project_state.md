...
| SSL positive pairs (lead-lag) | [`baselines/graph_builder_positive_pairs.py`](baselines/graph_builder_positive_pairs.py) | `admin1_lead_lag_v0`: same stripped `admin1_code`, `first_seen` gap in **[32, 90]** days inclusive (cross-month min; ~quarter max). Still **admin1-local** — cross-regional propagation needs predictive coding per roadmap. |
| BagEncoder (weighted sparse pool) | [`baselines/graph_builder_bag_encoder.py`](baselines/graph_builder_bag_encoder.py) | `RetrievedGraphBatch` sparse edges → row-wise weighted, L2-normalized `[B, 128]`; optional one-hop parameter-free message passing. |
