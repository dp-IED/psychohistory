...
- **Parallel tracks:** **WM v0 on events** does not wait for Polymarket; markets add **label contracts**, **masking**, and **coverage audits** once the loop is real. Rerun the **France** reference when shared code changes—do not collapse the program into a single protest demo metric. **Iran** (or similar contested geographies) is a **stress-test / shadow** lane after the GRU ablation driver is stable—not the primary benchmark that drives iteration until baselines and eval contracts are frozen (**`next_steps.md` §2.3**).
