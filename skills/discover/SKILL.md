...
3. Open at most `K` new problems under `## Problems`. Each needs **Motivation** and **Resolution** (`YYYY-MM-DD`, after today). Prefer farther **resolution day** as the overlay strengthens. No quality gate. Do not add dated claims. Do not write **claim** or **justification**. If opening more than one problem this tick, mix domains *and* mix horizon/evidence regime per `references/discovery.md` (domains: at least three, none over half the batch; horizons: at least one near ≤30 days and one far ≥90 days; evidence: at least one news-now and one analog/base-rate; weighed against what is already open). Do not open a problem whose resolution day is already past. Completion: new problem count is ≤ `K`; existing claims are untouched.
