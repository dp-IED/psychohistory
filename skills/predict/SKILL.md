...
This tick writes forecasts. Workers follow `references/structure.md` for the Structure block and `references/analog-prior.md` when a card matches. On an administrative tariff or trade clock they read `references/cases/tariff-proclamation-deadline-delay.md`, score delay separately from a signed deal, and treat an already-extended / post-pause date as take-effect unless a new delay instrument appears. On a delayed majority primary runoff they read `references/cases/majority-primary-runoff.md` and start from the matching phase (wide first-round gap vs GOP MAGA consolidator; the named consolidator outranks a non-consolidator plurality). On a consultative or first-step EU accession-talks referendum in an EEA/fisheries state they read `references/cases/eea-fisheries-eu-talks-referendum.md` and start from capital-Yes / coast-No (talks≠membership is not a Yes prior). On a junta- or military-transition constitutional yes/no they read `references/cases/junta-constitutional-referendum.md`, score held separately from official Yes, and treat a CNE-named later proclamation date as the resolution day (polling Sunday is not the certified-result clock). On a sitting statewide incumbent facing a named same-party challenger in a first-past-the-post intra-party primary they read `references/cases/incumbent-plurality-primary.md` and start from the incumbent when late polls remain double-digit (age copy, challenger spend, and a House anti-incumbent wave are not falsifiers; this is not a delayed majority runoff and not an open seat). If that delay names a new public date, slide this problem’s **resolution day** later so the post-pause phase stays live and can be scored. The day’s claim is then that new clock’s modal (take-effect unless a further delay instrument exists), not a pause recap. Open problems on a discover tick; that procedure is `references/discovery.md`. Do not invent a second heading for a short pause of a clock that is still the same question.
