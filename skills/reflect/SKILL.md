...
4. Write vault graph material that the grades earned: entities, threads, or concepts under `graph-vault/` following `references/vault.md`. Completion: each selected claim has at least one vault write, or an explicit note that the vault already held the fact.
