...
3. Group selected claims by problem. Grade the **whole series**: every **Claim** and **Justification**, including the **Structure** block (`references/structure.md`). The earliest matching **claim** is the prize; a last-day-only hit is not the same as early skill. A last-day scrape that ignored a good analog is a method miss. Be proactive: if the series shows a missing method, add overlay rather than waiting for a later tick. Completion: you can state a verdict for the series (what transferred, what failed, which forecast day first matched, whether the analog class earned a card).
